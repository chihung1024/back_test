# --- 完整版 update_data.py（2025-07-17 修正版）---
...
DATA_DIR = Path("data"); PRICES_DIR = DATA_DIR / "prices"
...
def main():
    t0 = time.time()
    sp, nd = set(get_sp500_list()), set(get_nasdaq100_list())
    tickers = sorted(sp | nd)
    if not tickers:
        print("❌ 無成分股來源"); return
    ...
    with ThreadPoolExecutor(MAX_WORKERS) as ex:
        for fut in tqdm(as_completed({ex.submit(fetch_fundamentals,t):t for t in tickers}),
                        total=len(tickers), desc="Fundamentals"):
            d = fut.result()
            if d: fundamentals.append(d)
    ...
    success = set()
    with ThreadPoolExecutor(MAX_WORKERS) as ex:
        for fut in tqdm(as_completed({ex.submit(fetch_price_history,t):t for t in tickers}),
    ...
    # 與舊檔比對
    old = pd.read_json(JSON_PATH, orient='records') if JSON_PATH.exists() else pd.DataFrame()
    new = pd.DataFrame(fundamentals)
    if not old.equals(new):
        new.to_json(JSON_PATH, orient='records', indent=2)
        print(f"✅ Update finished in {time.time()-t0:.1f}s")
    else:
        print("ℹ️ 成分股 & 基本面無變動，跳過提交")
