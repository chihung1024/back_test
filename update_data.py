...
def fetch_history(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            df = yf.download(
                ticker,
                start=start_date,  # <-- 使用傳入的 start_date
                progress=False,
                auto_adjust=True
            )
...
def main():
    ...
    success = set()
    with ThreadPoolExecutor(MAX_WORKERS) as ex:
        # 將 start_date 傳遞給 fetch_history
    ...
    if not new_frames:
        ...
        if existing_df is not None:
            ...
            # 關鍵：刪除重複的索引，並保留最後一個（即最新的）
            final_df = combined[~combined.index.duplicated(keep="last")]
        else:
            final_df = new_df
        
        print(f"💾 正在寫入合併後的 Parquet 檔案，總筆數: {len(final_df)}")
